...
extern "C" void krnl_jpeg(ap_uint<AXI_WIDTH>* jpeg_pointer,
                                  const int size,
                                  ap_uint<64>* yuv_mcu_pointer,
                                  ap_uint<32>* infos) {
// clang-format off
#pragma HLS INTERFACE m_axi port = jpeg_pointer     depth = 65000 offset = slave bundle = gmem \
					  latency = 64 num_read_outstanding = 32 max_read_burst_length = 32
#pragma HLS INTERFACE m_axi port = yuv_mcu_pointer 	depth = 230400 offset = slave bundle = gmem \
					  latency = 64 num_write_outstanding = 32 max_write_burst_length = 32
#pragma HLS INTERFACE m_axi port = infos 			depth = 1024   offset = slave bundle = gmem \
					  latency = 64 num_write_outstanding = 32 max_write_burst_length = 32
